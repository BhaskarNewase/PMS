...
projectRoute.route('/current-user-project-list/:id').get((req, res) => {
  Project.find({ userId: req.params.id}, (error, data) => {
    if (error) {
      return next(error)
    } else {
      res.json(data)
...
// Get single project
projectRoute.route('/read-project/:id').get((req, res, next) => {
